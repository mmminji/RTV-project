...
if __name__ == '__main__':
    ...
    for idx, pth in enumerate(list(mapping_dict.values())):
        ...
        if len(pth['csv_file']) == 1 :
            ...
            structure_tmp = structure_tmp.append(pd.read_csv(pth['csv_file'][0]), ignore_index = True)
            ...
            simul_col = [s for s in structure_tmp.columns if 'SIMULATIONTIME' in s]
            for i in simul_col:
                column_year_name = 'year' + "_" + i
                column_time_name = 'time' + "_" + i
                column_bef_aft = 'aft' + "_" + i
                structure_tmp[column_year_name] = structure_tmp[[s for s in structure_tmp.columns if 'SIMULATIONTIME' in s]].loc[:,i].str.split(" ").str[0]
                time_tmp = structure_tmp[[s for s in structure_tmp.columns if 'SIMULATIONTIME' in s]].loc[:,i].str.split(" ").str[1]
                change_tmp = pd.to_datetime(time_tmp)
                change_tmp[change_tmp.dt.hour>12] = change_tmp[change_tmp.dt.hour>12] - dt.timedelta(hours = 12)
                #structure_tmp[column_time_name] = pd.Series(['PM' if int(i[0:2]) >= 12 else 'AM' for i in time_tmp]) + " " + change_tmp.dt.strftime('%H:%M:%S')
                structure_tmp[column_bef_aft] = pd.Series(['PM' if int(i[0:2]) >= 12 else 'AM' for i in time_tmp])
                structure_tmp[column_time_name] = change_tmp.dt.strftime('%H:%M:%S')
            
            structure_all = structure_all.append(structure_tmp.drop(simul_col, inplace = False, axis = 1), ignore_index=True)
            ...
            for i, file in enumerate(pth['csv_file']):
                ...
                if i == 0:
                    ...
                    concat_structure = pd.merge(concat_structure, pd.read_csv(file),
                                        ...
                                        how = "inner")
            structure_tmp = structure_tmp.append(concat_structure, ignore_index = True)
            ...
            for i in simul_col:
                ...
                column_time_name = 'time' + "_" + i
                column_bef_aft = 'aft' + "_" + i
                structure_tmp[column_year_name] = structure_tmp[simul_col].loc[:,i].str.split(" ").str[0]

                time_tmp = structure_tmp[simul_col].loc[:,i].str.split(" ").str[1]
                change_tmp = pd.to_datetime(time_tmp)
            ...
            structure_all = structure_all.append(structure_tmp.drop(simul_col, inplace = False, axis = 1))
